...
namespace MeraStore.Shared.Common.Http
{
    ...
    : IApiClient
  {
    ...
    private readonly MaskingService _maskingService =
      maskingService ?? throw new ArgumentNullException(nameof(maskingService));
    ...
    private async Task<string> IndexDocumentAndGetUrl(string content, string documentType)
    {
      ...
      var documentUrl =
        $"{elasticUrl}/request-response-logs/_doc/{response.Id}?_source=content"; // Adjust the URL as needed
